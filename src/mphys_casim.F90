...
!        , qg(nz,nx,1), ng(nz,nx,1), m3g(nz,nx,1), cfliq(nz,nx,1)
...
! REAL(wp) :: AitkenSolBk(nz,nx,1)
! REAL(wp) :: AccumSolBk(nz,nx,1) 
! REAL(wp) :: CoarseSolBk(nz,nx,1)
...
!   CALL mphys_init(its, ite, jts, jte, kts, kte, ims, ime, jms, jme, kms, kme, l_tendency=.true.)
...
!     if ( pswitch%l_pcond ) casdiags % l_pcond = .TRUE.
!     if ( pswitch%l_psedl ) then
!        casdiags % l_psedl = .TRUE.
!        casdiags % l_surface_rain = .TRUE.
!        casdiags % l_precip = .TRUE.
!     endif
!     if ( pswitch%l_praut ) casdiags % l_praut = .TRUE.
!     if ( pswitch%l_pracw ) casdiags % l_pracw = .TRUE.
!     if ( pswitch%l_prevp ) casdiags % l_prevp = .TRUE.
...
!        if ( pswitch%l_phomc ) casdiags % l_phomc = .TRUE.
!        if ( pswitch%l_pinuc ) casdiags % l_pinuc = .TRUE.
!        if ( pswitch%l_pidep ) casdiags % l_pidep = .TRUE.
!        if ( pswitch%l_piacw ) casdiags % l_piacw = .TRUE.
!        if ( pswitch%l_pisub ) casdiags % l_pisub = .TRUE.
!        if ( pswitch%l_pimlt ) casdiags % l_pimlt = .TRUE.
!        if ( pswitch%l_psedi ) then
!           casdiags % l_psedi = .TRUE.
!           casdiags % l_surface_snow = .TRUE.
!           casdiags % l_precip = .TRUE.
!        endif
!        if ( pswitch%l_psmlt ) casdiags % l_psmlt = .TRUE.
!        if ( pswitch%l_psaut ) casdiags % l_psaut = .TRUE.
!        if ( pswitch%l_psaci ) casdiags % l_psaci = .TRUE.
!        if ( pswitch%l_psacw ) casdiags % l_psacw = .TRUE.
!        if ( pswitch%l_psacr ) casdiags % l_psacr = .TRUE.
!        if ( pswitch%l_pssub ) casdiags % l_pssub = .TRUE.
!        if ( pswitch%l_psdep ) casdiags % l_psdep = .TRUE.
!        if ( pswitch%l_pseds ) then
!           casdiags % l_pseds = .TRUE.
!           casdiags % l_surface_snow = .TRUE.
!           casdiags % l_precip = .TRUE.
!        endif
!        if ( pswitch%l_pgacw ) casdiags % l_pgacw = .TRUE.
!        if ( pswitch%l_pgacs ) casdiags % l_pgacs = .TRUE.
!        if ( pswitch%l_pgmlt ) casdiags % l_pgmlt = .TRUE.
!        if ( pswitch%l_pgsub ) casdiags % l_pgsub = .TRUE.
